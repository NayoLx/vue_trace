				...
				<section class="form_contianer">
					...
					<el-form ref="loginForm" :model="user" :rules="rules">
						<el-form-item prop="name">
							<el-input type="text" v-model="user.name" placeholder="用户名"></el-input>
						</el-form-item>
						<el-form-item prop="password">
							<el-input type="password" v-model="user.password" placeholder="密码"></el-input>
						</el-form-item>
						<el-select v-model="user.server" placeholder="默认服务器" style="width: 100%">
							<el-option v-for="item in servers" :key="item.server" :label="item.server"
								:value="item.server">
							</el-option>
						</el-select>
						<div class="keep-pass">
	...
	export default {
		...
		methods: {
			...
			login() {
				...
				if (this.user.server == 'fake') {
					...
					setTimeout(async () => {
						...
						this.user.accounts = getData().account;
						this.user.account = this.user.accounts[0];
			...
			async initExchange() {
				...
				if (exchange == null) {
					if (this.user.server == 'fake') {
						exchange = getData().exchange;
						this.$session.set("exchange", exchange, 1);
					} else {
			...
			async initProduct() {
				...
				if (product == null) {
					if (this.user.server == 'fake') {
						...
						this.$session.set("product", product, 1);
					} else {
			...
			async initContract() {
				...
				if (contract == null) {
					if (this.user.server == 'fake') {
						...
						this.$session.set("contract", contract, 1);
					} else {
