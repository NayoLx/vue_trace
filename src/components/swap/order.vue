<template>
	<div class="main-order">
			...
			<div class="form-row">
				<div class="form-row-item" style="margin-right: 10px">
					<div class="form-row-item-title-f">
						...
						<Icon :type="isReadOnly == true ? 'md-lock' : 'md-unlock'" size="17"
							@click="changeReadOnly()" />
					</div>
					<el-select v-model="form.swap" filterable remote v-bind:disabled="isReadOnly" reserve-keyword
				...
				<div class="form-row-item">
					...
					<el-input-number v-model="form.head" controls-position="right" :min="1" :max="10"></el-input-number>
				</div>
			</div>
			<div class="form-row">
				<div class="form-row-item">
					...
					<el-input-number v-model="form.price" controls-position="right" :step="100" style="width: 120px"
				...
				<div class="form-row-item" style="margin: 0px 10px">
					...
					<el-select v-model="form.side" placeholder="请选择">
						<el-option v-for="item in sides" :key="item" :label="item" :value="item">
						</el-option>
					</el-select>
				</div>
				<div class="form-row-item">
					...
					<el-select v-model="form.openFlag" placeholder="请选择">
						<el-option v-for="item in openFlags" :key="item" :label="item" :value="item">
						</el-option>
					</el-select>
				</div>
			</div>
			<div class="form-row">
				<div class="form-row-item">
					...
					<el-select v-model="form.hedgeFlag" placeholder="请选择">
						<el-option v-for="item in hedgeFlags" :key="item" :label="item" :value="item">
						</el-option>
					</el-select>
				</div>
				<div class="form-row-item" style="margin: 0px 10px">
					...
					<el-select v-model="form.ordType" placeholder="请选择">
						<el-option v-for="item in ordTypes" :key="item" :label="item" :value="item">
						</el-option>
					</el-select>
				</div>
				<div class="form-row-item">
					...
					<el-select v-model="form.tif" placeholder="请选择">
						<el-option v-for="item in tifs" :key="item" :label="item" :value="item">
						</el-option>
					</el-select>
				</div>
			</div>
			<div style="text-align: center; margin: 25px 0">
				<el-button type="primary" @click="onSubmit" style="width: 60%">下单</el-button>
			</div>
		</Form>
		<div>
		...
		<el-dialog title="选择合约" :visible.sync="swapVisible" width="50%" height="500" v-dialogDrag>
			<template>
				<div class="choose-swap">
						...
						<el-col :span="18">
							<div class="choose-swap-tree">
								...
								<el-table :data="tableData" style="width: 100%" height="250px"
									:header-cell-style="cellStyle" :cell-style="rowStyle" @row-click="onClick">
					...
					<div class="choose-swap-footer">
						...
						<div class="choose-swap-footer-btn">
							<el-button type="primary" plain size="mini" @click="onChooseSubmit()">确定</el-button>
		...
		<!-- 操作确认  -->
		<el-dialog title="操作确认" :visible.sync="submitVisible" width="350px" v-dialogDrag>
			<template>
				<div class="col_submit">
					<p class="col_title">确定下单 <span class="col_tip">{{form.swap}}</span> 吗？</p>
					<div class="col_list">
						<span class="col_child">{{form.openFlag}}</span>
						<span class="col_child">{{form.price}}</span>
						<span class="col_child">{{form.side}}</span>
						<span class="col_child">{{form.head}}手</span>
						<span class="col_child">{{form.hedgeFlag}}</span>
						<span class="col_child">{{form.ordType}}</span>
					</div>
					<div class="col_btn">
