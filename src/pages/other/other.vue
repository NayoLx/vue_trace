...
-->
			...
			<div class="center">
				...
				<div class="imgtable">
					<div class="handler">
						...
						<span @click="changeImgTable('s')"
							:class="{active:currentImgTable==='s'}">{{$t("exchange.depth")}}</span>
					</div>
								...
								<div class="bd bd_limited" v-show="!showMarket">
									<Form ref="formValidate">
										...
										<div class="slider-wrap">
											...
											<div class="slider-stop" v-for="item in sliderStep" v-bind:key="item"
												:style="'left: '+item+'%;'"
										...
										<Button v-if="exchangeable == 1" class="bg-green"
											@click="buyWithLimitPrice">{{$t("exchange.buyin")}}({{currentCoin.coin}})</Button>
										<Button v-else
							...
							<div class="panel panel_sell">
								...
								<div class="bd bd_limited" v-show="!showMarket">
									<Form ref="formValidate">
										...
										<Button v-if="exchangeable == 1" class="bg-red"
											@click="sellLimitPrice">{{$t("exchange.sellout")}}({{currentCoin.coin}})</Button>
										<Button v-else
											class="bg-gray">{{$t("exchange.sellout")}}({{currentCoin.coin}})</Button>
									</Form>
								</div>
								<div class="bd bd_market" v-show="showMarket">
									<Form ref="formValidate">
			...
			<div class="left plate-wrap" style="position:relative;">
				<div class="lightning-panel" v-if="showCountDown" :style="{background:countDownBgColor}">
					<img v-if="lang == 'zh_CN' && publishType=='FENTAN'"
						src="../../assets/images/lightning-bg.png">
					<img v-if="lang == 'en_US' && publishType=='FENTAN'"
						src="../../assets/images/lightning-bg-en.png">
					<img v-if="lang == 'zh_CN' && publishType=='QIANGGOU'"
						src="../../assets/images/lightning-bg2.png">
					<img v-if="lang == 'en_US' && publishType=='QIANGGOU'"
						src="../../assets/images/lightning-bg2-en.png">
					<div class="l-content">
						...
						<p class="l-info">
							<span class="l-txt">{{$t("exchange.publishamount")}}：</span><span
								class="l-count">{{publishAmount | toFixedPublishAmount}} </span><span
								class="l-unit">{{currentCoin.coin}}</span> &nbsp;&nbsp;&nbsp;&nbsp;
							<span class="l-txt">{{$t("exchange.publishprice")}}：</span><span
								class="l-price">{{publishPrice | toFixedPublishPrice}} </span><span
				...
				<div class="plate-nowprice">
					<span class="price"
						:class="{buy:currentCoin.change>0,sell:currentCoin.change<0}">{{currentCoin.price | toFixed(baseCoinScale)}}</span>
		...
		<div class="order">
			<div class="order-handler">
				...
				<span @click="changeOrder('history')"
					:class="{active:selectedOrder==='history'}">{{$t('exchange.hisdelegation')}}</span>
												...
												click: () => {
													...
													if (this.isLogin) {
														if (
															...
															this.cancelCollect(params.index, params
																.row);
															event.currentTarget.className ==
																"ivu-icon ivu-icon-ios-star-outline";
														} else {
					...
					columns: [{
						...
						},
						{
							...
							render(h, params) {
								return h(
									"span",
									params.row.type === "LIMIT_PRICE" ? self.$t("exchange.limited_price") : self
									.$t("exchange.market_price")
								);
							}
						},
						{
		...
		methods: {
			...
			getKline() {
				...
				let config = {
					...
					library_path: process.env.NODE_ENV === "production" ?
						"../assets/charting_library/" : "../src/assets/js/charting_library/",
					...
					.then(response => {
						...
						if (resp.bid && resp.bid.items) {
							...
							if (resp.bid.items.length < this.plate.maxPostion) {
								for (
									var i = resp.bid.items.length; i < this.plate.maxPostion; i++
								) {
									var bid = {
										price: 0,
										amount: 0
